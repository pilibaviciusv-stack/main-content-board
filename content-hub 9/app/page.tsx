...
function getPipelineIcon(name: string) {
  ...
  if (lower.includes('ig') || lower.includes('instagram') || lower.includes('story') || lower.includes('stories')) return <Layers size={14} />;
  return <Video size={14} />;
}

const isYoutubePipeline = (name: string) =>
  name.toLowerCase().includes('youtube') || name.toLowerCase().includes('yt');
        ...
        <div style={{ padding: '20px 16px 16px', borderBottom: '1px solid #1e2130' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: '#475569' }}>Danas · Vabanque</div>
            </div>
          </div>
        </div>
        ...
        <div style={{ flex: 1, padding: '12px 8px', overflowY: 'auto' }}>
          ...
          {state.pipelines.map(p => (
            <div key={p.id}>
              ...
              {isYoutubePipeline(p.name) && navItem('Roadmap', <Map size={12} />, view.type === 'roadmap' && (view as any).pipelineId === p.id, () => setView({ type: 'roadmap', pipelineId: p.id }), true)}
            </div>
          ))}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <button onClick={() => {
                ...
                const newCard = {
                  ...
                  type: 'Top of Funnel' as const, editor: '', format: '', scheduledDate: '', cost: '',
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: 24 }}>
          ...
          {view.type === 'pipeline' && activePipeline && (
            <Board pipeline={activePipeline} cards={state.cards} users={state.users}
              onCardsChange={cards => setState(prev => prev ? { ...prev, cards } : prev)} />
          )}
          ...
          {view.type === 'music' && (
            <MusicBank tracks={state.musicBank}
              onChange={musicBank => setState(prev => prev ? { ...prev, musicBank } : prev)} />
          )}
          {view.type === 'footage' && (
            <FootageLinks items={state.footageLinks}
              onChange={footageLinks => setState(prev => prev ? { ...prev, footageLinks } : prev)} />
          )}
          {view.type === 'inspiration' && (
            <InspirationProfiles
              profiles={(state as any).inspirationProfiles || []}
              onChange={inspirationProfiles => setState(prev => prev ? { ...prev, inspirationProfiles } as any : prev)} />
          )}
          {view.type === 'settings' && (
            <PipelineSettings pipelines={state.pipelines}
              onChange={pipelines => setState(prev => prev ? { ...prev, pipelines } : prev)} />
          )}
        </div>
      </div>
    </div>
