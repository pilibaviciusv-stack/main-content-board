...
export default function Board({ pipeline, cards, users, onCardsChange }: Props) {
  ...
  const handleDragStart = (start: any) => setDraggingId(start.draggableId);
  ...
  const deleteCard = (id: string) => onCardsChange(cards.filter(c => c.id !== id));
  ...
  return (
    <>
                      ...
                      {stageCards.map((card, index) => (
                        <Draggable key={card.id} draggableId={card.id} index={index}>
                          {(provided, snapshot) => (
                            ...
                            >
                              <div style={{ fontSize: 13, fontWeight: 600, color: '#e2e8f0', lineHeight: 1.4, marginBottom: 10 }}>
                                {card.title}
                              </div>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                                <span style={{
                                  fontSize: 10, fontWeight: 700, padding: '2px 7px', borderRadius: 4,
                                  background: TYPE_COLORS[card.type] + '22', color: TYPE_COLORS[card.type],
                                  textTransform: 'uppercase', letterSpacing: '0.06em'
                                }}>{card.type.split(' ')[0]}</span>
      ...
      {selectedCard && (
        <CardModal card={selectedCard} users={users} onSave={saveCard} onDelete={deleteCard} onClose={() => setSelectedCard(null)} />
      )}
    </>
  );
}
