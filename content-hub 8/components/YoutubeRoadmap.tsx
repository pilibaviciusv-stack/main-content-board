...
export default function YoutubeRoadmap({ pipeline, cards, users, onCardsChange }: Props) {
  ...
  const saveCard = (updated: ContentCard) => onCardsChange(cards.map(c => c.id === updated.id ? updated : c));
  const deleteCard = (id: string) => { onCardsChange(cards.filter(c => c.id !== id)); setSelectedCard(null); };

  const CARD_W = 340;
  const CARD_H = 110;
  const COL_GAP = 120; // horizontal gap between left and right columns
  const ROW_GAP = 32;  // vertical gap between cards
  const CONNECTOR_W = 80; // width of the curve connector

  // Zigzag: even index = left side, odd index = right side
  // Centre line is between the two columns
  const LEFT_X = 0;
          ...
          <div style={{ position: 'relative', width: RIGHT_X + CARD_W, margin: '0 auto' }}>
            ...
            >
              {pipelineCards.map((card, i) => {
                ...
                // Start point: right edge of left card or left edge of right card
                const x1 = isLeft ? LEFT_X + CARD_W : RIGHT_X;
                const x2 = nextIsLeft ? LEFT_X + CARD_W : RIGHT_X;

                // Curve through the center
                const cx = isLeft ? RIGHT_X - 20 : LEFT_X + CARD_W + 20;
                const midY = (y1 + y2) / 2;
                  ...
                  >
                    ...
                    >
                      ...
                      <div style={{
                        ...
                        background: card.thumbnail ? '#000' : titleToGradient(card.title),
                      }}>
                        {card.thumbnail ? (
                          ...
                          <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: 0.3 }}>
                            <svg width="28" height="28" viewBox="0 0 24 24" fill="none">
                              <polygon points="5,3 19,12 5,21" fill="#e2e8f0" />
                            </svg>
                          </div>
                        )}
                        {/* Status dot */}
                      ...
                      <div style={{ flex: 1, padding: '12px 14px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minWidth: 0 }}>
                        <div>
                          <div style={{ fontSize: 13, fontWeight: 700, color: '#e2e8f0', lineHeight: 1.35, marginBottom: 4, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            {card.title}
                          </div>
                          {card.hook && (
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6 }}>
                          <span style={{ fontSize: 10, fontWeight: 700, color: status.color, background: status.color + '18', padding: '2px 7px', borderRadius: 4, textTransform: 'uppercase', letterSpacing: '0.06em', whiteSpace: 'nowrap' }}>
                            {status.label}
                          </span>
                          {dateLabel && (
                            <span style={{ fontSize: 10, color: isPosted ? '#22c55e' : '#64748b', fontWeight: 600, whiteSpace: 'nowrap' }}>
                              {dateLabel}
                            </span>
                          )}
                        </div>
